...
1. **Clone repo**
   ...
   ```
2. **Arrange building environments**
   ...
   **Windows**
   ***Automatic script***
   The automatic script is provided to install needed environments automatically:
   ...
   ***Configure manually***
   What's more, you can install needed environments manually. It's supposed `winget` is available at your computer and its version is at least `v1.4.10052`. Theoretically `scoop` or `choco` works as well but we can't promise there won't be problems.

   First install Virtual Studio Building Tools. If you already have it installed, be careful about the version.

   ```
   winget install --id Microsoft.VisualStudio.2022.BuildTools
   ```

   Then it will come to `rustup`, Node.JS, `LLVM`.
   TODO
